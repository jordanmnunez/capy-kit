...
export interface Thread{id:string;projectId:string;title:string|null;titleCustom:boolean;status:ThreadStatus;archived:boolean;lastModelId:string|null;usage:Usage;createdAt:string;updatedAt:string;lastActivityAt:string}
...
export interface CreateThreadBody{requestId:string;projectId:string;message:string;title?:string;model?:{modelId:string;reasoningMode?:string;modes?:{fast?:boolean}};machineSize?:"small"|"medium"|"large"|"ultra"|"hyper"|"bigguy"}
export interface SendMessageBody{text:string;delivery?:"interrupt"|"queue"|"steer";model?:CreateThreadBody["model"]}
export interface Task{id:string;threadId:string;parentId:string|null;taskPath:string;projectId:string;title:string;status:"working"|"waiting"|"idle"|"done"|"failed";usage:Usage;createdAt:string;updatedAt:string}
