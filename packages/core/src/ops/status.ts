...
export const status = defineOp({ name: "status", summary: "List project threads and current statuses.", effect: "read", input: z.object({ status: z.enum(THREAD_STATUSES).optional(), limit: z.coerce.number().int().min(1).max(100).optional() }), output: z.unknown(), async run(a, c) { const projectId = requireProject(undefined, c); const p = await resources(c).threads.list({ ...a, projectId }); return { projectId, count: p.items.length, threads: p.items }; } });
