...
export const threadsList = defineOp({
  ...
  input: z.object({ status: z.enum(THREAD_STATUSES).optional(), limit: z.coerce.number().int().min(1).max(100).optional(), cursor: z.string().optional() }),
  output: page,
...
const control = (name: string, summary: string, method: "interrupt" | "archive" | "unarchive") => defineOp({ name, summary, effect: "mutate" as const, input: z.object({ id: z.string().min(1) }), output: z.unknown(), run: (a, c) => resources(c).threads[method](a.id) });
...
export const threadsMessage = defineOp({ name: "threads.message", summary: "Send a message to a thread.", effect: "create", input: z.object({ id: z.string().min(1), text: z.string().min(1), delivery: z.enum(["interrupt", "queue", "steer"]).optional() }), output: z.object({ id: z.string(), deduped: z.boolean() }), run: (a, c) => resources(c).threads.message(a.id, { text: a.text, delivery: a.delivery }) });
