...
describe("current Capy API",()=>{
 it("uses the new base URL and creates idempotently with requestId/message/projectId",async()=>{const {fetch,calls}=makeMockFetch(()=>({json:thread}));const out=await delegate.run({message:"fix it",requestId:"request-1"},testContext({fetch}));expect(out.threadId).toBe("jam_1");expect(calls[0]?.path).toBe("/api/v1/threads");expect(calls[0]?.body).toEqual({requestId:"request-1",message:"fix it",projectId:"proj_test"});});
 it("uses cursor pagination, projectId, and text/delivery messages",async()=>{let n=0;const {fetch,calls}=makeMockFetch(c=>{if(c.path.endsWith("/message"))return{json:{id:"01K",deduped:false}};n++;return{json:{items:[{...thread,id:`jam_${n}`}],cursor:n===1?"next":null}}});const c=testContext({fetch});expect((await threadsList.run({},c)).cursor).toBe("next");expect(calls[0]?.query.get("projectId")).toBe("proj_test");await threadsMessage.run({id:"jam_1",text:"focus",delivery:"steer"},c);expect(calls[1]?.body).toEqual({text:"focus",delivery:"steer"});});
