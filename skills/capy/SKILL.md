...
# → { threadId, url, status, runState, model, reasoning }   (the thread is created AND started)
...
#   steer message with --model/--reasoning; --attachmentUrls <url>; --json → { id, status:"sent" }
```
