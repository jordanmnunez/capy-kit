...
Updated **2026-08-24**. The Capy public API is an organization-key-scoped runtime surface at `https://api.capy.ai/api/v1`. Its authoritative contract is vendored in `spec/capy.openapi.json`; the current snapshot has **33 paths and 39 operations**. `README.md` describes what can be run now, and `SPEC.md` records the architectural target.
...
- poll a thread with `wait`, summarize project work with `status`, discover and inspect accessible projects, read organization usage, list organization users, and organize threads with folders and pins;
...
The registry has **35 Ops**, including the mechanical `delegate`, `wait`, and `status` composites. `capy init` and `capy config projects` remain CLI-only local configuration flows; `capy projects` is the faithful public project API projection.

## Current public API boundary

The public specification exposes 39 HTTP operations:
...
- Project discovery is organization-scoped. Local aliases remain optional conveniences for selecting a default project or friendly `--project` name.
